...
def inexact_newton_cg(eval_J, eval_gradient, build_hessian_operator,
    ...
    last_step_stalled = False  # did the previous line search collapse to ~0 step?
    ...
    for k in range(max_outer_iter):
        ...
        # NOTE: distinguish TRUE ftol convergence (a full step was taken and
        # J barely improved because we are at a minimum) from a STALL (the
        # previous line search collapsed to alpha~0 and could not reduce J).
        # A stall with a still-large gradient is NOT convergence; reporting it
        # as such hides a failed optimization (exactly what happened when the
        # data term was too weak and the step direction was unreliable).
        if J_prev is not None and ftol > 0:
            rel_dJ = (J_prev - J_x) / max(abs(J_prev), abs(J_x), 1.0)
            print(f"  rel_dJ       : {rel_dJ:.6e}  (ftol = {ftol:.2e})")
            if rel_dJ <= ftol:
                stalled = (last_step_stalled and g_norm > 10.0 * grad_tol)
                if stalled:
                    print(f"  STOPPING: line search STALLED (no progress) while "
                          f"||grad||_inf {g_norm:.2e} >> gtol {grad_tol:.1e}. "
                          f"This is NOT convergence -- the step direction could "
                          f"not reduce J. Likely an ill-conditioned/weak-data "
                          f"Hessian; check the data precision (noise_std).")
                    reason = "line_search_stall"
                else:
                ...
                history.append({
                    ...
                    "cg_iters": 0, "cg_reason": "ftol_check",
        ...
        # a line search that used all its backtracks and collapsed to a
        # negligible step made essentially no progress -> flag it so the
        # ftol check next iteration can tell a stall from real convergence
        last_step_stalled = (n_backtracks >= 20) and (alpha < 1e-6)
