...
def model_verify_hippylib_style(Hop, eval_J, eval_gradient, CC_map,
    ...
    # -------------------------------------------------------------------
    # random test direction h (hippylib: parRandom.normal(1., h))
    # -------------------------------------------------------------------
    ...
    if plot:
        try:
            ...
            # use the first VALID (non-NaN) point to anchor the slope-1
            # reference line, since the largest eps values may have
            # failed (forward solver did not converge) and are NaN
            valid_grad = np.where(~np.isnan(err_grad))[0]
            valid_H = np.where(~np.isnan(err_H))[0]

            fig, axes = plt.subplots(1, 2, figsize=(11, 5))

            axes[0].loglog(eps, err_grad, "-ob", label="FD error")
            if len(valid_grad) > 0:
                i0 = valid_grad[0]
                axes[0].loglog(eps, eps * (err_grad[i0] / eps[i0]), "-.k",
                                label="slope 1 (theoretical)")
            axes[0].set_xlabel("eps")
            axes[0].set_ylabel("error")
            axes[0].set_title("FD Gradient Check")
            axes[0].legend()

            axes[1].loglog(eps, err_H, "-ob", label="FD error")
            if len(valid_H) > 0:
                i0 = valid_H[0]
                axes[1].loglog(eps, eps * (err_H[i0] / eps[i0]), "-.k",
                                label="slope 1 (theoretical)")
            axes[1].set_xlabel("eps")
            axes[1].set_ylabel("error (L-inf norm)")
            axes[1].set_title("FD Hessian Check")
            axes[1].legend()
    ...
    return eps, err_grad, err_H, symm_err
