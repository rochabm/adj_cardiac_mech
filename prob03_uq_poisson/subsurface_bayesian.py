...
-----------------------
Adaptation of the hIPPYlib tutorial 3_SubsurfaceBayesian into
...
# =============================================================================
# EXPERIMENT GUIDE
# Copy this block to the top of subsurface_bayesian.py (after the docstring)
# Each experiment requires changing only 1-2 lines. Run and compare fig1-fig6.
# =============================================================================
#
# CURRENT DEFAULTS (tutorial values):
#   ntargets  = 50          # number of sensors
#   targets_y = uniform(0.1, 0.5)  # sensors in bottom half only
#   rel_noise = 0.01        # 1% noise level
#   gamma     = 0.1         # prior correlation strength
#   delta     = 0.5         # prior mass term
#   theta0    = 2.0         # correlation length along principal axis
#   theta1    = 0.5         # correlation length perpendicular
#   alpha     = pi/4        # orientation of anisotropy (45 degrees)
#   nx = ny   = 64          # mesh resolution
#   GN_ITER   = 5           # Gauss-Newton warmup iterations
#   k_eig     = 50          # number of eigenvectors for UQ
#
# -----------------------------------------------------------------------------
# EXPERIMENT 1 -- Number of sensors
# Change: ntargets
# Expected: more sensors -> lower posterior variance, more data-informed modes,
#           better MAP reconstruction, more eigenvalues above lambda=1 in fig2
#
#   ntargets = 10    # very few: poor reconstruction, high uncertainty everywhere
#   ntargets = 50    # current (tutorial default)
#   ntargets = 200   # many: good reconstruction, low uncertainty
#
# -----------------------------------------------------------------------------
# EXPERIMENT 2 -- Sensor placement
# Change: targets_x, targets_y
# Expected: variance reduction (fig4) concentrates where sensors are placed
#
#   # current: bottom half only
...
#
#   # top half only
#   targets_y = rng.uniform(0.5, 0.9, ntargets)
#
#   # clustered in one corner
#   targets_x = rng.uniform(0.1, 0.4, ntargets)
#   targets_y = rng.uniform(0.1, 0.4, ntargets)
#
# -----------------------------------------------------------------------------
# EXPERIMENT 3 -- Noise level
# Change: rel_noise
# Expected: high noise -> prior dominates, MAP near zero, few eigenvalues > 1
#           low noise  -> data dominates, sharp MAP, many eigenvalues > 1
#
#   rel_noise = 0.001   # very low:  data dominates, sharp MAP
#   rel_noise = 0.01    # current (1%)
#   rel_noise = 0.1     # high:      prior dominates, MAP close to prior mean
#   rel_noise = 0.5     # very high: MAP essentially prior mean (zero)
#
# -----------------------------------------------------------------------------
# EXPERIMENT 4 -- Prior parameters
# Change: gamma, delta, theta0, theta1, alpha
# Expected: changes spatial correlation structure of m_true and MAP
#
#   # stronger regularization (shorter correlation, more uncertain)
#   gamma = 0.01;  delta = 5.0
#
#   # weaker regularization (longer correlation, more smoothing)
#   gamma = 1.0;   delta = 0.1
#
#   # isotropic prior (no diagonal stripe -- remove anisotropy)
#   theta0 = 1.0;  theta1 = 1.0
#
#   # different stripe orientation
#   alpha = 0.0          # horizontal correlation
#   alpha = math.pi/2    # vertical correlation
#   alpha = math.pi/4    # current: 45 degree diagonal
#
# -----------------------------------------------------------------------------
# EXPERIMENT 5 -- Mesh resolution
# Change: nx, ny
# Expected: finer mesh -> better MAP resolution
#           key property: number of data-informed modes (eigenvalues > 1)
#           stays roughly CONSTANT -- independent of mesh size
#           this validates the scalability of the algorithm
#
#   nx = ny = 32    # coarse: 1089 parameter dofs, fast
#   nx = ny = 64    # current: 4225 parameter dofs
#   nx = ny = 128   # fine:  16641 parameter dofs, slow
#
# -----------------------------------------------------------------------------
# EXPERIMENT 6 -- Newton vs Gauss-Newton
# Change: GN_ITER
# Expected: GN_ITER=0  (full Newton from start): may diverge far from MAP
#           GN_ITER=5  (current): stable warmup, then quadratic convergence
#           GN_ITER=25 (always GN): stable everywhere, slower near MAP
#           observe convergence curve shape in fig6
#
#   GN_ITER = 0     # always full Newton
#   GN_ITER = 5     # current (tutorial default)
#   GN_ITER = 25    # always Gauss-Newton
#
# -----------------------------------------------------------------------------
# EXPERIMENT 7 -- Number of eigenvectors for UQ
# Change: k_eig
# Expected: k_eig too small -> negative posterior variance entries (clipped)
#           k_eig > effective rank -> adding noise, not signal
#           effective rank = number of eigenvalues > 1 in fig2
#           rule of thumb: k_eig should exceed the effective rank
#
#   k_eig = 10    # low rank: fast but inaccurate variance
#   k_eig = 50    # current (tutorial default)
#   k_eig = 100   # higher rank: more accurate, slower eigensolver
#
# -----------------------------------------------------------------------------
# SUGGESTED EXPERIMENT ORDER (for learning):
#   1. ntargets = 10 vs 200     -- most dramatic visual impact
#   2. rel_noise = 0.001 vs 0.1 -- shows prior/likelihood balance
#   3. theta0=theta1=1 (isotropic) -- shows what anisotropy adds
#   4. nx=ny=32 vs 128          -- validates mesh-independence property
#   5. GN_ITER = 0 vs 25        -- Newton vs Gauss-Newton tradeoffs
#
# To compare runs: rename the output folder before each run, e.g.:
#   mkdir run_ntargets10 && mv fig*.png run_ntargets10/
# =============================================================================
...
m_true_arr = prior.sample(rng)

m_true = fem.Function(Va, name="m_true")
m_true.x.array[:] = m_true_arr
